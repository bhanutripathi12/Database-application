...
function create(opts) {
    if (opts === void 0) { opts = {}; }
    var _a = opts.compiler, compiler = _a === void 0 ? require('typescript') : _a, _b = opts.cachePath, cachePath = _b === void 0 ? path_1.default.resolve(__dirname, '../cache') : _b;
    ...
    var transpileModule = function (input, transpileOptions) {
        ...
        if (canCache) {
            // compute cache keys
            var config = tslib_1.__assign({ version: compiler.version }, transpileOptions.compilerOptions);
            var configSha1 = sha1_1.computeSha1(Buffer.from(JSON.stringify(config), 'utf8'));
            var codeSha1 = sha1_1.computeSha1(Buffer.from(transpileOptions.fileName || '', 'utf8'), Buffer.from(input, 'utf8'));
